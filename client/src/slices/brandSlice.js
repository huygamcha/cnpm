...
const getDetailBrands = createAsyncThunk("/brands/getDetail", async (id) => {
  const config = {
    headers: {
      "Content-type": "application/json",
      Authorization: `Bearer ${currentUser.token}`,
  ...
  const { data } = await axios.get("/api/brands/getBrandById", { id }, config);
  return data;
});
...
const brandSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    //get detail
    builder.addCase(getDetailBrands.pending, (state) => {
      state.loading = true;
      state.error = "";
    });
    builder.addCase(getDetailBrands.fulfilled, (state, action) => {
      state.loading = false;
      state.brand = action.payload;
    });
    builder.addCase(getDetailBrands.rejected, (state, action) => {
      state.loading = false;
      state.brand = action.error.message;
    });
